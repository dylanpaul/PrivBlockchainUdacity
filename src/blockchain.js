...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //this following part of code was adapted from mentor Q/A
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((currentTime - timemsg) <= (5 * 60)){
                ...
                reject("Submission over 5 minutes.");
            }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            for (let index = 1; index < self.chain.length; index++) {
                ...
                try {
                    ...
                    if(data.owner === address.toString()){
                        stars.push(data);
                    }
                } catch (error) {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let index = 0; index < self.chain.length; index++) {
                ...
                if(!valid){
                    errorLog.push("Error at Block ${block.height}: block tampered");
                }  
                //check previous hash
                if(block.height > 0){
                    let previousBlock = self.chain[index - 1];
                    if(block.previousBlockHash !== previousBlock.hash){
                        errorLog.push("Error at Block ${block.height} because previous hash doesn't match");
                    }
                }
